...
passport_1.default.use(new passport_local_1.Strategy({
    ...
    try {
        ...
        if (response?.role === user_interface_1.UserRole.DRIVER) {
            response = await driver_model_1.Driver.findOneAndUpdate({ user: user._id }, { $set: { driverStatus: river_interface_1.DriverStatus.AVAILABLE } }, { new: true }).populate("user", "email name role location lastOnlineAt");
        }
        if (!response) {
